...
- **Data Source**:
  ...
  - **Location**: `individuals/mutant/individual_XX.json` → `cells` array
...
- **Data Source**:
  ...
  - **Location**: `individuals/mutant/individual_XX.json` → `cell_history` dictionary
...
- **Data Source**:
  ...
  - **Transformation**: Load snapshot cells → create PetriDish → call calculate_gene_jsd() → get 20 values → plot histogram
...
- **Data Source**:
  ...
  - **Location**: `individuals/mutant/individual_XX.json` → `metadata.gene_jsds`
...
- **Data Source**:
  ...
  - **Location**: `individuals/mutant/individual_XX.json` → `cell_history`
...
## 3. Cell-level Methylation

### a) Snapshot Histogram 🆕 ADD
**What this plot shows**: For all cells in a snapshot, we calculate what fraction of each cell's sites are methylated (e.g., 30 out of 100 = 0.30), then plot how many cells have each methylation level.

- **X-axis**: Methylation proportion (scalar, range 0-1, continuous)
- **Y-axis**: Count of cells (integer, range 0 to total cells in snapshot)
- **Data Source**:
  - **Attribute**: `cell.methylated` - a list of 0s and 1s, one for each CpG site
  - **Object**: Each Cell in the snapshot
  - **Location**: `snapshots/year30_snapshot.json` → each cell's `methylated` array
  - **Computation**: Count the 1s in the methylated array, divide by array length
  - **Transformation**: For each cell: `proportion = sum(methylated) / len(methylated)`
...
### b) Batch Comparison 🆕 ADD
**What this plot shows**: For each individual, we calculate the average methylation proportion across all its cells, then compare these averages between batches.
...
- **Data Source**:
  ...
  - **Location**: `individuals/mutant/individual_XX.json` → `cells` → each cell's `methylated`
...
| **Gene-level JSD** | 🆕 Add (20 points) | ✅ Exists | 🆕 Add (mean+median) | 🆕 Add (mean+median) |
| **Cell Methylation** | 🆕 Add | 🆕 Add | ✅ Exists | ✅ Exists |
